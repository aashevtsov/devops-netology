...
1. Для выполнения курсовой работы был  установлен дистрибутив Ubuntu Server 21.10. ВМ была развёрнута под Vmware ESXi. ВМ был дан ip-адрес 10.10.10.11. Работа с ней будет осуществляться с рабочего места Windows, c адресом в этой же сети.
...
Rule added (v6)
```
На Компьютере с которого мы будем пробовать открывать веб на этом сервере пропишем запись в файл hosts: 10.10.10.11 test.example.com
